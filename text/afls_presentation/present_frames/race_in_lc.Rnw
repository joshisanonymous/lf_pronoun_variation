\begin{frame}{La race dans les études du créole précédentes\Logo}
  ...
  \only<3->{
    Et les pronoms...
    \begin{itemize}
      \item Les Blancs produisent des pronoms plus francisés (\lexi{avec elle} et pas \lexi{avec li (f.)}) \smallcite{\parencite{klingler_if_2003}}
      \begin{itemize}
    ...
    \begin{itemize}
      \item Les Blancs emploient moins d'affriquées [tʃ] et [dʒ] parce qu'ils les qualifient comme <<n***e>> \smallcite{\parencite[p.~91]{neumann_creole_1985}}
    \end{itemize}
  }
\end{frame}