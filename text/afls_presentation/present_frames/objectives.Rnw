\begin{frame}{Objectif\Logo}
  De mieux comprendre la façon dont la race \emph{et l'ethnicité} sont exprimées par les pronoms sujets en français louisianais, \alert{dans une perspective large} de la langue. \\
  \vspace{0.25cm}
  % \begin{enumerate}
  ...
  % \end{enumerate}
  \vspace{1.5cm}
  {\footnotesize * Tous les noms sont anonymisés.}
\end{frame}