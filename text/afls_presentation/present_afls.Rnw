...
\documentclass{beamer}
  ...
  % Research questions
  \newcommand{\RQone}{Has the Black-White racial binary of the US in general been superimposed on ethnic categories in Louisiana to the extent that they are reproduced in 3rd person subject pronouns?}
...
\begin{document}
  ...
  \section{Discussion}
    % Participants discussing how people speak differently
    % Participants discussing what Cajuns and Creoles are
  \section{Conclusion}
    % There's some muddiness both in racial identities -- on both sides -- but also generally a racial division in 3pl usage
  %   <<child = "./present_frames/answers.Rnw">>=
  %   @
  <<child = "./present_frames/fractal_recursivity.Rnw">>=
  @
